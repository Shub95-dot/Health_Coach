...
class HealthCalculator:
    ...
    @staticmethod
    def estimate_tdee(bmr: float, experience: str, goal: str = None) -> int:
    ...
    @staticmethod
    def get_macro_split(goal: str, calories: int) -> Dict[str, int]:
        ...
        return {
            "protein_g": int((calories * split["protein"]) / 4),
            "carbs_g": int((calories * split["carbs"]) / 4),
            "fat_g": int((calories * split["fat"]) / 9)
        }
    ...
    def select_exercises(self, 
        ...
        for pattern in ["squat", "push_horizontal", "pull_horizontal"]:
            if not selected[pattern]:
                selected[pattern] = self._select_from_pattern(all_exercises, pattern, 1)
...
class PlanGenerator:
    ...
    def _generate_nutrition_section(self, profile, goal: str) -> str:
        ...
        if all([weight, height, age, sex]):
            ...
            output += f"Protein: {macros['protein_g']}g | Carbs: {macros['carbs_g']}g | Fat: {macros['fat_g']}g\n"
